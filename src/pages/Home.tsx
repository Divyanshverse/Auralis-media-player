...
const TrackCard = memo(({ 
  ...
  return (
    <div
      className="w-[140px] md:w-[180px] p-3 md:p-4 bg-[#141416] hover:bg-[#1f1f22] rounded-lg transition-colors group cursor-pointer flex-shrink-0 snap-start"
    ...
    >
      <div className="relative mb-3 md:mb-4 shadow-[0_8px_24px_rgba(0,0,0,0.5)]">
        ...
        <button
          ...
          className={cn(
            "absolute bottom-2 right-2 w-10 h-10 md:w-12 md:h-12 bg-[#A78BFA] rounded-full flex items-center justify-center shadow-xl hover:scale-105 hover:bg-[#8B5CF6] transition-all z-10",
            isCurrent && isPlaying ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0"
          )}
        >
          {isCurrent && isPlaying ? (
            <Pause className="w-5 h-5 text-[#0B0B0D] fill-current" />
          ) : (
...
const TrackListItem = memo(({ 
  ...
  return (
    <div
      className="flex items-center justify-between group cursor-pointer hover:bg-[#1f1f22] p-2 rounded-xl transition-colors"
      onClick={() => navigate(`/song/${track.id}`, { state: { track } })}
    >
      ...
      <button
        ...
        className={cn(
          ...
          isCurrent && isPlaying ? "bg-[#A78BFA] text-[#0B0B0D]" : "bg-[#1f1f22] text-white group-hover:bg-[#A78BFA] group-hover:text-[#0B0B0D]"
        )}
      >
        {isCurrent && isPlaying ? (
...
export default function Home() {
  ...
  const renderSkeletons = () => (
    <div className="flex overflow-x-auto gap-4 pb-4 px-4 md:px-6 hide-scrollbar">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="w-[140px] md:w-[180px] p-3 md:p-4 bg-[#141416] rounded-lg animate-pulse flex-shrink-0">
          <div className="w-full aspect-square bg-[#1f1f22] rounded-md mb-4"></div>
          <div className="h-4 bg-[#1f1f22] rounded w-3/4 mb-2"></div>
          <div className="h-3 bg-[#1f1f22] rounded w-1/2"></div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="pt-4 md:pt-6 pb-40 h-full overflow-y-auto overflow-x-hidden bg-[#0B0B0D]">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 mb-8 bg-transparent py-3 -mt-4 md:-mt-6">
        ...
        >
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#A78BFA] to-[#67E8F9] flex items-center justify-center text-[#0B0B0D] font-bold text-xl shadow-[0_0_15px_rgba(167,139,250,0.5)]">
        ...
        <div className="flex items-center gap-3 ml-4 shrink-0">
          <button 
            className="w-10 h-10 rounded-full bg-[#1f1f22] flex items-center justify-center text-white hover:bg-[#383838] transition-colors"
        ...
        <div className="flex gap-3 overflow-x-auto hide-scrollbar">
          <button 
            ...
            className={cn(
              ...
              activeCategory === 'All' ? "bg-[#A78BFA] text-[#0B0B0D]" : "bg-[#1f1f22] text-white hover:bg-[#383838]"
            )}
          >
            All
          </button>
          <button 
            ...
            className={cn(
              "px-6 py-2 rounded-full font-semibold whitespace-nowrap transition-colors",
              activeCategory === 'Playlists' ? "bg-[#A78BFA] text-[#0B0B0D]" : "bg-[#1f1f22] text-white hover:bg-[#383838]"
            )}
          >
            Playlists
          </button>
          <button 
            ...
            className={cn(
              "px-6 py-2 rounded-full font-semibold whitespace-nowrap transition-colors",
              activeCategory === 'Liked Songs' ? "bg-[#A78BFA] text-[#0B0B0D]" : "bg-[#1f1f22] text-white hover:bg-[#383838]"
            )}
          >
            Liked Songs
          </button>
          <button 
            ...
            className={cn(
              "px-6 py-2 rounded-full font-semibold whitespace-nowrap transition-colors",
              activeCategory === 'Downloaded' ? "bg-[#A78BFA] text-[#0B0B0D]" : "bg-[#1f1f22] text-white hover:bg-[#383838]"
            )}
          >
            Downloaded
          </button>
      ...
      <div className="mb-8 px-4 md:px-6">
        ...
        >
          <div className="z-10 w-2/3">
            <h3 className="text-2xl font-bold text-[#0B0B0D] mb-2">Discover Weekly</h3>
            <p className="text-[#0B0B0D]/80 text-sm mb-6">The Original slow instrumental<br/>best Playlists</p>
              ...
              >
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  {playlist.tracks.length > 0 ? (
                    ...
                    <div className="w-14 h-14 rounded-lg bg-[#1f1f22] flex items-center justify-center flex-shrink-0">
                ...
                <button 
                  ...
                  className="w-8 h-8 rounded-full bg-[#1f1f22] flex items-center justify-center text-white group-hover:bg-[#A78BFA] group-hover:text-[#0B0B0D] transition-colors flex-shrink-0 ml-4"
                >
                  <Play className="w-4 h-4 fill-current ml-0.5" />
                </button>
              </div>
