...
export default function Search() {
  ...
  return (
    <div className="p-6 pb-24 h-full overflow-y-auto bg-[#121212]">
      <div className="sticky top-0 z-10 bg-[#121212] pt-2 pb-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            ...
            {['Podcasts', 'Live Events', 'Made For You', 'New Releases', 'Pop', 'Hip-Hop'].map((category, i) => (
              <div
                key={i}
                className="aspect-square rounded-lg p-4 relative overflow-hidden cursor-pointer hover:scale-105 transition-transform"
                style={{ backgroundColor: `hsl(${i * 60}, 70%, 40%)` }}
              >
                <h3 className="text-white font-bold text-xl">{category}</h3>
                <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-black/20 rounded-full rotate-45"></div>
              </div>
            ))}
          </div>
        )}
